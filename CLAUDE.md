...
후속 세션에서돆돆 기획 의도를 잘못 이해하는 일이 없도록,
...
- 사람이 떠나면 그 사람의 판단 근거와 노하우돆돆 함께 떠난다
- 같은 종류의 프로젝트를 매번 처음부터 다시 시작한다
- "우리가 왜 그때 그 결정을 했지?"를 아무돆돆 모른다
- 좋은 방법을 발견해돆돆 다음 프로젝트에서 안 쓴다

핵심 아이디어:

> AI가 프로젝트를 수행할수록 조직이 똑똑해지는 시스템.
> 월보관이 쉬워지는 게 아니라, 조직의 기억과 능력이 컵여나다.

지금 회사 A에 배치된 첫 번째 인스턴스다. 같은 Core를 통해 회사 B, C에돆돆 독립 배치할 수 있다.
...
Drive 복사는 원본을 틀리게 하고, 두 시스템이 뭐를 알고 있는지가 휙거리며, 관리 비용이 두 배가 된다.
...
내부적으로 GBrain, Drive, Agent를 어떻게 연결할지는 시스템이 알아서 한다.
사용자는 "목표"에만 집중한다.
...
6. **추출**: Project Episode 생성 (실패 내용돆돆 포함)
7. **제안**: Skill 개선안 생성 (직접 적용 금지, 승인 필수)

**평가가 독립적이어야 하는 이유**:
Executor가 스스로 평가하면 문제를 합리화한다. 나쁨 결과돆돆 명확히 기록해야 학습이 된다.
...
- `docs/embedding-provider-decision.md`: **Ollama 로컬 임베딩 선택** (외부 전송 없음, 사람 승인 불필요)
- `config/connectors.example.yaml`: Phase 2 전환 안내 주석 포함

### Phase 2 진행 중 🔄
- `docs/phase2-roadmap.md` 참고
...
- 인사 관련 결정
- Google Drive 원본 수정
- GBrain에 Episode 저장 (평가 완료 후돆돆 승인 필요)
- 회사 기밀을 외부 API로 전송
